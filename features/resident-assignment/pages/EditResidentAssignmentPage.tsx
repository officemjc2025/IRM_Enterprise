...
export default function EditResidentAssignmentPage({ params }: EditResidentAssignmentProps) {
  ...
  useEffect(() => {
    const loadOptionsAndDetails = async () => {
      try {
        const [unitsRes, peopleRes, detailsRes] = await Promise.all([
          fetch("/api/v1/units"),
          fetch("/api/v1/persons"),
          fetch(`/api/v1/residents/${id}`)
        ]);
        ...
        if (detailsJson.success) {
          ...
          setUnitId(d.unit_id);
          setPersonId(d.person_id);
          setOccupancyType(d.occupancy_type);
          setPrimaryResident(d.primary_resident);
          setMoveInDate(d.move_in_date);
          setMoveOutDate(d.move_out_date || "");
          setStatus(d.status);
          setRemark(d.remark || "");
        } else {
          setError(detailsJson.message);
        }
      } catch (err) {
        console.error("Failed to load options and details:", err);
      ...
      <div className="max-w-xl mx-auto space-y-6">
        <div>
          <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200">
            {t.residentAssignment.edit}
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            {language === "en" ? "Update assignment dates, status, or type." : "ปรับปรุงข้อมูลการมอบหมาย สถานะ หรือประเภทการอยู่อาศัย"}
          </p>
        </div>
